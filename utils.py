...
def check_trigger_functions(message):
    hands_off_keyword = os.getenv("KEYWORD_HANDS_OFF")

    similarity = fuzz.ratio(message.lower(), hands_off_keyword.lower())

    if similarity >= float(os.environ.get('KEYWORD_SIMILARITY_THRESHOLD')) or hands_off_keyword.lower() in message.lower():
        message = re.sub(hands_off_keyword, '', message, flags=re.IGNORECASE)
        # Se la somiglianza è sopra la soglia, chiama la funzione
        return ("hands_off", message)
    else:
